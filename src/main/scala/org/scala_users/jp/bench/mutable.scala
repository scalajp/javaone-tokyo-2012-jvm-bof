...
sealed abstract class RBTree {
  ...
  def put(key: String, value: String): RBTree = {
    val result = p(key, value)
    result.color = Black
    result
  }
  def p(key: String, value: String): RBTree
  def height: Int
}
class Node(var color: Int, var key: String, var value: String, var left: RBTree, var right: RBTree) extends RBTree {
  ...
  def p(k: String, v: String) = {
    val c = k compare key
    if      (c < 0) left = left.p(k, v)
    else if (c > 0) right = right.p(k, v)
    else            value = v
...
object Empty extends RBTree {
  ...
  def p(key: String, value: String) = new Node(Red, key, value, this, this)
  val height = 0
}
